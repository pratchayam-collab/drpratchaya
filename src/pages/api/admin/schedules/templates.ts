...
import { createTemplate, deleteTemplate, listTemplates } from '~/lib/admin/schedules';
...
export const GET: APIRoute = async (context) => {
  ...
  const templates = await listTemplates(env.DB, locationId);
...
export const POST: APIRoute = async (context) => {
  ...
  if (auth instanceof Response) return auth;
  const body = await readJson<Parameters<typeof createTemplate>[2]>(context.request);
  const id = await createTemplate(auth, env.DB, body);
...
export const DELETE: APIRoute = async (context) => {
  ...
  await deleteTemplate(auth, env.DB, id);
  return json({ ok: true });
};
